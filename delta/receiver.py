...
BASE_URL = os.getenv("MEDIA_BASE_URL", "https://media.topkit.app")
...
async def receive_upload(
    ...
    kit_type: str = Query(None),   # ex: "home", "away", "third", "gk"
    league_id: str = Query(None),  # pour les versions uniquement
    ...
    # ── Naming convention ──────────────────────────────────────────────────────
    if folder == "master_kit":
        # master_{kit_type}_{uid}.ext
        if kit_type:
            parts = ["master", kit_type, uid]
        else:
            parts = ["master", uid]

    elif folder == "version":
        # version_{kit_type}_{league_id}_{uid}.ext
        if kit_type and league_id:
            parts = ["version", kit_type, league_id, uid]
        elif kit_type:
            parts = ["version", kit_type, uid]
        else:
            parts = ["version", uid]

    else:
        # {folder}_{entity_id}_{side}_{uid}.ext  (comportement original)
        parts = [folder]
        if entity_id:
