...
# Configuration
BASE_URL = "https://maillot-trading.preview.emergentagent.com/api"
...
class Phase2AdminVerificationTester:
    ...
    def test_admin_cannot_remove_from_collections(self):
        ...
        try:
            ...
            # Get a jersey for collection removal test
            jerseys_response = self.session.get(f"{self.base_url}/jerseys?limit=1")
            if jerseys_response.status_code != 200 or not jerseys_response.json():
                self.log_test("Admin Collection Remove Restriction", "FAIL", "No jerseys available for testing")
                return False
