...
def calculate_estimated_price(master_kit: dict, collection_item: dict = None) -> float:
    ...
    try:
        ...
        if collection_item:
            ...
            # Check if player coefficient was pre-calculated by the endpoint
            if '_player_coefficient' in collection_item:
                player_coefficient = collection_item['_player_coefficient']
                coefficients += player_coefficient
                if player_coefficient > 0:
                    logger.info(f"Applied pre-calculated player coefficient {player_coefficient}")
            else:
                # Fallback: Use associated_player_id if present
                ...
                if associated_player_id:
                    ...
                    # Use a default coefficient for associated players
...
async def get_collection_item_price_estimation(
    ...
    try:
        ...
        # PLAYER COEFFICIENT FIX: Look up player coefficient before price calculation
        player_coefficient = 0.0
        associated_player_id = collection_item.get('associated_player_id')
        if associated_player_id:
            ...
            if player:
                ...
                if player.get("influence_coefficient"):
                    player_coefficient = player["influence_coefficient"]
                
                # Priority 2: Use player_type coefficient
                elif player.get("player_type"):
                    try:
                        player_type = PlayerType(player["player_type"])
                        player_coefficient = get_player_type_coefficient(player_type)
                    except ValueError:
        ...
        # Add player coefficient to collection item temporarily for calculation
