...
# ── Ligues cibles : (apifootball_league_id, region, scope, scoring_weight) ───
# UEFA
TARGET_LEAGUES = [
    # UEFA — ligues domestiques top 5 (= ton CSV)
...
def fetch_league(league_id: int) -> dict | None:
    """Appelle API-Football et retourne le premier résultat."""
    try:
        resp = httpx.get(
            f"{API_BASE}/leagues",
            headers=HEADERS,
...
def build_doc(target: dict, api_data: dict | None, now: str) -> dict:
    """Construit le document MongoDB à partir des données cibles + API."""
...
def run():
    ...
    for target in TARGET_LEAGUES:
        ...
        if existing:
            if DRY_RUN:
                print(f"[UPDATE dry] {name} (id={league_id}) → déjà en base, serait mis à jour")
            else:
                api_data = fetch_league(league_id)
                time.sleep(0.3)  # rate-limit
                doc = build_doc(target, api_data, now)
                doc.pop("league_id", None)   # ne pas écraser l'id existant
                doc.pop("created_at", None)  # conserver la date de création
                col.update_one(
        ...
        api_data = fetch_league(league_id)
        time.sleep(0.3)  # rate-limit doux (300ms entre chaque appel)
        ...
        doc = build_doc(target, api_data, now)
        col.insert_one(doc)
        print(f"[INSERT] {name} (id={league_id}) → logo={doc['logo_url'][:40] if doc['logo_url'] else 'n/a'}")
        inserted += 1

    print(f"\n{'='*60}")
    if DRY_RUN:
        print(f"Dry-run : {inserted} à insérer, {updated} à mettre à jour, {errors} erreurs.")
