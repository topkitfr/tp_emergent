...
export default function VersionDetail() {
  ...
  // Players for this kit/version (AJOUT)
  const [players, setPlayers] = useState([]);

  const fetchVersion = () => {
    setLoading(true);
    Promise.all([getVersion(versionId), getVersionEstimates(versionId)])
      .then(([vRes, eRes]) => {
  ...
  // Fetch players (AJOUT)
  const fetchPlayers = (kitIdOrVersionId) => {
    if (!kitIdOrVersionId) return;
    getKitPlayers(kitIdOrVersionId)
  ...
  // Quand la version est chargée, on peut appeler les players (AJOUT)
  useEffect(() => {
    if (version) {
      // On utilise master_kit.kit_id si tu as branché l’endpoint comme ça,
      // sinon tu peux décider d’utiliser version.version_id côté backend.
      const masterId = version.master_kit?.kit_id || version.master_kit?.id;
      fetchPlayers(masterId || version.id);
    }
  }, [version]);
  ...
  const mk = version?.master_kit || null;
  const seasonYear = parseSeasonYear(mk?.season);
  const estimateChartData =
    ...
    <div className="animate-fade-in-up">
      ...
      <div className="border-b border-border px-4 lg:px-8 py-3">
        <div className="max-w-7xl mx-auto flex items-center gap-2 text-sm text-muted-foreground flex-wrap">
          ...
          <span className="text-foreground truncate">
            {version.competition} - {version.model}
          </span>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 lg:px-8 py-8 lg:py-12">
        <div className="grid md:grid-cols-[1fr_1.2fr] gap-8 lg:gap-16">
          ...
          <div className="space-y-4">
            <div
              className="aspect-[3/4] border border-border bg-card overflow-hidden"
              data-testid="version-front-image"
            >
              <img
                src={proxyImageUrl(version.front_photo || mk?.front_photo)}
                alt="Front"
            ...
            {version.back_photo && (
              <div
                className="aspect-[3/4] border border-border bg-card overflow-hidden"
                data-testid="version-back-image"
              >
                <img
                  src={proxyImageUrl(version.back_photo)}
                  alt="Back"
                  className="w-full h-full object-cover"
                />
              </div>
          ...
          <div className="space-y-6">
            <div>
              ...
              {mk && (
                ...
                >
                  <Link
                    to={`/teams/${mk.club_id ?? mk.club_slug ?? mk.club}`}
                    className="hover:text-primary transition-colors"
                  >
                    {mk.club}
                  </Link>{' '}
                  - {mk.season} - {mk.kit_type}
                </p>
              )}
              <h1
                className="text-3xl sm:text-4xl tracking-tighter leading-none mb-2"
            ...
            <div className="grid grid-cols-2 gap-6">
              <div>
                <div className={fieldLabel} style={fieldStyle}>
                  Competition
                </div>
                <div className="flex items-center gap-2 mt-1">
                  <Package className="w-4 h-4 text-primary" />
                  <span className="text-sm">{version.competition || 'None'}</span>
                </div>
              </div>
              <div>
                <div className={fieldLabel} style={fieldStyle}>
                ...
                <div className="flex items-center gap-2 mt-1">
                  <Package className="w-4 h-4 text-primary" />
                  <span className="text-sm">{version.model || 'None'}</span>
                </div>
              </div>
              <div>
                <div className={fieldLabel} style={fieldStyle}>
                ...
                <div className="flex items-center gap-2 mt-1">
                  ...
                  <span className="text-sm font-mono">{version.sku_code || 'None'}</span>
                </div>
              </div>
              <div>
                <div className={fieldLabel} style={fieldStyle}>
                ...
                <div className="flex items-center gap-2 mt-1">
                  <Hash className="w-4 h-4 text-primary" />
                  <span className="text-sm font-mono">{version.ean_code || 'None'}</span>
                </div>
              </div>
              <div>
                <div className={fieldLabel} style={fieldStyle}>
            ...
            {estimates && estimates.count > 0 && (
              <div className="border border-border p-4" data-testid="estimation-stats">
                ...
                >
                  VALUE ESTIMATION
                  <span className="font-mono text-xs text-muted-foreground ml-2 normal-case">
            ...
            {user && (
              <div className="flex flex-wrap gap-2">
                ...
                <Button
                  ...
                  data-testid="request-removal-btn"
                >
            ...
            {showAddForm && user && (
              <div
                className="border border-primary/30 p-4 space-y-4"
                data-testid="add-collection-form"
              >
                <h4 className="text-sm uppercase tracking-wider" style={fieldStyle}>
                  ADD TO COLLECTION
                </h4>
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <div className="space-y-1">
                      ...
                      <Select
                        value={flockingType || 'none'}
                        onValueChange={v => setFlockingType(v === 'none' ? '' : v)}
                      >
                        <SelectTrigger
                          className={inputClass}
                          data-testid="collection-flocking-type"
                        >
                    ...
                    <div className="space-y-1">
                      ...
                      <Select
                        value={flockingOrigin || 'none'}
                        onValueChange={v => setFlockingOrigin(v === 'none' ? '' : v)}
                      >
                        <SelectTrigger
                          className={inputClass}
                          data-testid="collection-flocking-origin"
                        >
                          <SelectValue placeholder="None" />
                        </SelectTrigger>
                        <SelectContent className="bg-card border-border">
                    ...
                    <div className="space-y-1">
                      ...
                      <Input
                        ...
                        onChange={e => setFlockingDetail(e.target.value)}
                        placeholder="e.g., Messi 10"
                        className={inputClass}
                        data-testid="collection-flocking-detail"
                      />
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    ...
                    <Select
                      value={conditionOrigin || 'none'}
                      onValueChange={v => setConditionOrigin(v === 'none' ? '' : v)}
                    >
                      <SelectTrigger
                        className={inputClass}
                        data-testid="collection-condition-origin"
                      >
                  ...
                  <div className="space-y-1">
                    ...
                    <Select
                      value={physicalState || 'none'}
                      onValueChange={v => setPhysicalState(v === 'none' ? '' : v)}
                    >
                      <SelectTrigger
                        className={inputClass}
                        data-testid="collection-physical-state"
                      >
                        <SelectValue placeholder="Select" />
                      </SelectTrigger>
                      <SelectContent className="bg-card border-border">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    ...
                    <Select
                      value={collectionSize || 'none'}
                      onValueChange={v => setCollectionSize(v === 'none' ? '' : v)}
                    >
                      <SelectTrigger
                        className={inputClass}
                        data-testid="collection-size-select"
                      >
                        <SelectValue placeholder="Select" />
                      </SelectTrigger>
                      <SelectContent className="bg-card border-border">
                  ...
                  <div className="space-y-1">
                    ...
                    <Input
                      ...
                      data-testid="collection-purchase-cost"
                    />
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  <div className="flex items-center gap-4">
                    <div className="flex items-center gap-2">
                      <Switch
                        ...
                        data-testid="collection-signed-switch"
                      />
                    ...
                    {signed && (
                      <div className="flex-1">
                        <Input
                          ...
                          placeholder="Signed by (player name)"
                          className={inputClass}
                          data-testid="collection-signed-by"
                        />
                      </div>
                    )}
                  </div>
                  {signed && (
                    <div className="flex items-center gap-2 ml-12">
                      <Switch
                        checked={signedProof}
                        onCheckedChange={setSignedProof}
                        data-testid="collection-signed-proof"
                      />
                      <Label
                ...
                <div className="flex gap-2">
                  ...
                  >
                    ...
                    {addingToCollection ? 'Adding...' : 'Confirm'}
                  </Button>
                  <Button
                    variant="outline"
                    onClick={() => setShowAddForm(false)}
                    className="rounded-none"
                    data-testid="cancel-add-collection-btn"
                  >
                    Cancel
                  </Button>
                </div>
            ...
            {showReportForm && user && (
              <div
                className="border border-destructive/30 p-4 space-y-3"
              ...
              >
                ...
                >
                  Suggest corrections for this version. Community will vote on
                  changes.
                </p>
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    ...
                    <Select
                      value={reportCorrections.competition || ''}
                      onValueChange={v =>
                        setReportCorrections(p => ({ ...p, competition: v }))
                      }
                    >
                      <SelectTrigger
                        className={inputClass}
                        data-testid="report-competition"
                      >
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent className="bg-card border-border">
                  ...
                  <div className="space-y-1">
                    ...
                    <Select
                      value={reportCorrections.model || ''}
                      onValueChange={v =>
                        setReportCorrections(p => ({ ...p, model: v }))
                      }
                    >
                      <SelectTrigger
                        className={inputClass}
            ...
            {showRemovalForm && user && (
              <div
                className="border border-destructive/50 p-4 space-y-3"
                data-testid="removal-form"
              >
                <h4
                ...
                <p
                  ...
                  style={{ fontFamily: 'DM Sans', textTransform: 'none' }}
                >
                  Request removal of this version. The community will vote on the
                  request.
                </p>
                <div className="space-y-1">
                  <Label className={fieldLabel} style={fieldStyle}>
                ...
                <div className="flex gap-2">
                  <Button
                    ...
                    className="rounded-none bg-destructive text-destructive-foreground hover:bg-destructive/90"
                    data-testid="submit-removal-btn"
                  >
                      ...
                      <div className="flex items-center gap-0.5">
                        {[1, 2, 3, 4, 5].map(s => (
                          <Star
                            key={s}
                            className={`w-3 h-3 ${
                              s <= r.rating ? 'text-accent fill-accent' : 'text-muted'
                            }`}
                          />
                        ))}
                      </div>
