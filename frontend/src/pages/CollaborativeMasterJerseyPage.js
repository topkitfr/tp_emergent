        ...
        <div className="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[80vh] overflow-y-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-4 p-4 bg-gray-50 rounded-lg">
              ...
              <div>
                ...
                <div className="flex items-center gap-4">
                  <input
                    type="file"
                    accept="image/*"
                    onChange={(e) => handleImageUpload('main_image', e.target.files[0])}
                    className="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100"
                  />
                  {imagePreviews.main_image && (
                    <div className="relative">
                      <img src={imagePreviews.main_image} alt="Aperçu maillot" className="w-12 h-16 object-cover rounded-lg border" />
                      <button
                        type="button"
                        onClick={() => {
                          setImageFiles(prev => ({ ...prev, main_image: null }));
                          setImagePreviews(prev => ({ ...prev, main_image: '' }));
                        }}
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  Images secondaires (dos, détails, variantes)
                  <span className="text-xs text-gray-500 ml-1">- Maximum 4 images</span>
                </label>
                
                {imageFiles.secondary_photos.length < 4 && (
                  <input
                    type="file"
                    accept="image/*"
