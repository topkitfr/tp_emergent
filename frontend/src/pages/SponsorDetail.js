...
export default function SponsorDetail() {
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      try {
        // Récupère le sponsor depuis la collection sponsors
        const sRes = await fetch(`/api/sponsors/${encodeURIComponent(decodedName)}`);
        const sData = sRes.ok ? await sRes.json() : null;
        setSponsor(sData);

        // Récupère les kits liés
    ...
    <div className="animate-fade-in-up" data-testid="sponsor-detail-page">
      ...
      <div className="border-b border-border px-4 lg:px-8 py-8">
        <div className="max-w-7xl mx-auto">
          <Link to="/database/sponsors" className="inline-flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground mb-4" data-testid="back-to-sponsors">
            <ArrowLeft className="w-3 h-3" /> Sponsors
          </Link>
          ...
          <div className="flex items-start gap-6">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-3">
                ...
                {/* Bouton Suggest Edit */}
                {isAdminOrMod && !isPending && sponsor && (
                  <TooltipProvider>
                    <Tooltip>
                      ...
                      {isPending && (
                        <TooltipContent><p>Waiting for approval</p></TooltipContent>
                      )}
                    </Tooltip>
                  </TooltipProvider>
                )}
      ...
      <div className="max-w-7xl mx-auto px-4 lg:px-8 py-8">
        <div className="flex flex-wrap gap-3 mb-6">
          <select value={filterSeason} onChange={e => setFilterSeason(e.target.value)}
            className="h-9 px-3 bg-card border border-border rounded-none text-sm" data-testid="sponsor-filter-season">
            <option value="">All Seasons</option>
            {seasons.map(s => <option key={s} value={s}>{s}</option>)}
          </select>
          <select value={filterTeam} onChange={e => setFilterTeam(e.target.value)}
            className="h-9 px-3 bg-card border border-border rounded-none text-sm" data-testid="sponsor-filter-team">
        ...
        {filteredKits.length === 0 ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 stagger-children" data-testid="sponsor-kits-grid">
      ...
      {showEdit && !isPending && sponsor && (
        <EntityEditDialog
          ...
          onSuccess={async () => {
            const res = await fetch(`/api/sponsors/${encodeURIComponent(decodedName)}`);
