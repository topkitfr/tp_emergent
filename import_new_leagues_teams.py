#!/usr/bin/env python3
"""
Script d'import des nouvelles ligues et équipes dans la base de données collaborative TopKit
Intègre les données fournies par l'utilisateur (France, Angleterre, Espagne, etc.)
"""
...
# Nouvelles données à importer (fournies par l'utilisateur)
NEW_LEAGUES_TEAMS_DATA = """France,Ligue 1,1,Paris Saint-Germain
...
class NewTopKitImporter:
    ...
    def parse_data(self) -> List[Tuple[str, str, int, str]]:
        ...
        entries = []
        for line in NEW_LEAGUES_TEAMS_DATA.strip().split('\n'):
            if line.strip():
                parts = line.split(',')
                if len(parts) == 4:
    ...
    async def create_competition_if_not_exists(self, pays: str, ligue: str, niveau: int) -> str:
        ...
        # Vérifier si déjà créée dans cette session
        ...
        try:
            ...
            async with self.session.post(
                ...
                if response.status == 200:
                    ...
                    # Vérifier différents messages d'erreur de doublon
                    if any(msg in error_detail for msg in ["existe déjà", "already exists", "duplicate"]):
                        print(f"ℹ️ Compétition existe: {ligue} ({pays})")
                        self.stats['competitions_skipped'] += 1
                        # Stocker comme "existing" pour éviter de refaire l'appel
                        self.competitions[league_key] = "existing_competition_id"
                        return "existing_competition_id"
                    else:
                        self.stats['errors'].append(f"Erreur création compétition {ligue}: {error_detail}")
                        return None
                else:
    ...
    async def create_team(self, equipe: str, pays: str, competition_id: str = None) -> bool:
        ...
        team_data = {
            "name": equipe,
            "country": pays,
            "league_id": competition_id if competition_id and competition_id != "existing_competition_id" else None,
            ...
            async with self.session.post(
                ...
                if response.status == 200:
                    ...
                    if any(msg in error_detail for msg in ["existe déjà", "already exists", "duplicate"]):
                        ...
                        self.stats['errors'].append(f"Erreur création équipe {equipe}: {error_detail}")
                        return False
                else:
                    error_text = await response.text()
    ...
    async def import_data(self):
        """Importe toutes les nouvelles données"""
        print("🚀 Début de l'import des nouvelles ligues et équipes TopKit...")
        print(f"📅 Date d'import: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        ...
        print(f"📊 {len(entries)} nouvelles entrées à traiter")
        ...
        print(f"🌍 Pays concernés: {', '.join(sorted(countries))}")
        print(f"🏆 Compétitions uniques: {len(unique_leagues)}")

        # Phase 1: Créer toutes les compétitions uniques
        print(f"\n📋 Phase 1: Création des {len(unique_leagues)} compétitions...")
        for i, (pays, ligue, niveau) in enumerate(sorted(unique_leagues), 1):
            print(f"[{i}/{len(unique_leagues)}] ", end="")
        ...
        for i, (pays, ligue, niveau, equipe) in enumerate(entries, 1):
            if i % 50 == 0 or i == len(entries):  # Progress update every 50 teams
                print(f"[{i}/{len(entries)}] Traitement des équipes...")
    ...
    def print_stats(self):
        ...
        print("\n" + "="*60)
        print("📊 STATISTIQUES D'IMPORT NOUVELLES DONNÉES TOPKIT")
        print("="*60)
        ...
        if self.stats['errors']:
            print(f"\n🔍 Détail des erreurs (dernières 10):")
            for error in self.stats['errors'][-10:]:  # Afficher les 10 dernières erreurs
                print(f"  • {error}")
            if len(self.stats['errors']) > 10:
                print(f"  ... et {len(self.stats['errors']) - 10} erreurs précédentes")
        ...
        total_attempts = total_processed + len(self.stats['errors'])
        success_rate = (total_processed / total_attempts) * 100 if total_attempts > 0 else 0
        
        print(f"\n🎯 Taux de succès: {success_rate:.1f}%")
        print(f"📈 Total traité: {total_processed}")
        print(f"🆕 Nouveaux éléments: {total_created}")
        print("="*60)

async def main():
    """Fonction principale"""
    try:
        async with NewTopKitImporter() as importer:
            success = await importer.import_data()
            importer.print_stats()
            
            if success:
                print("\n🎉 Import des nouvelles données terminé avec succès!")
                print("💡 Les équipes et compétitions ont été ajoutées à la base collaborative TopKit")
                return 0
            else:
                print("\n❌ Import échoué")
                return 1
